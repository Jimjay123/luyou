    ...
    <el-container>
      ...
      <el-main clearfix style="padding:0;">
        <div class="Analysis">
          ...
          <div class="corner" v-if="channelInfo.type == 11">频道</div>
          <div class="sandchannel" v-if="channelInfo.type == 12">沙盘频道</div>
          <span class="clock_minutes">
            <img src="../../assets/img/clock.svg" alt class="clock" />
            <span class="minutes">{{channelInfo.duration}}分钟</span>
          </span>
          <div class="fx">
            <h3>{{channelInfo.name||'标题未命名'}}</h3>
            <span class="oo1" v-if="channelInfo.type == 11">课程频道</span>
            <span class="oo1" v-if="channelInfo.type == 12">沙盘课程频道</span>
            ...
            <el-row>
              <el-button @click="Thirdparty(channelInfo)">去预览</el-button>
            </el-row>
          </div>
        </div>

        <div class="introduction" v-if="channelInfo.summary != undefined">
          <div class="jj">
        ...
        <div class="structure">
          ...
          <div class="ow" v-for="(item,index) in bundleList" :key="item.id">
            <div class="one">
              <span v-if="channelInfo.type == 11">第{{trimNumberFun(index+1)}}章</span>
              <p v-if="channelInfo.type == 11">{{item.bundleName}}</p>
              <div class="yll" v-for="items in item.courseList" :key="items.id">
                ...
                <el-row>
                  <el-button @click="Thirdpartys(items)">预览</el-button>
                </el-row>
              </div>
            </div>
          </div>
...
<script>
import {
  courseChannelDetail, //推荐频道/标准课详情页
  coursepartnerList //详情页师资介绍
} from "../../service/groupApi";
import { creatorCourseChannelDetail } from "../../service/teachersApi";
import { trimNumber, teaIroFun } from "../../utils/utils";
    ...
    Thirdparty(data) {
      ...
      if (data.type === 12) {
        ...
        if (num == -1) {
          ...
          //npe
          window.location.href = newUrl + "&preview=1";
        }
      }
    },
    // 课程结构处的预览
      ...
      if (type === 3 || type === 4) {
        ...
        this.$router.push({
          ...
          query: {
            ...
            address: address,
            displayR: true,
            refId: items.courseId
          }
        });
      } else {
        if (type === 2) {
          //npe
          window.location.href = items.courseServerUrl + "&preview=1";
        } else {
          //tpe
          window.open(
            window.location.href.split("#")[0] +
              "#/orthers?url=" +
              encodeURIComponent(items.courseServerUrl)
          );
        }
      }
      // window.open(
