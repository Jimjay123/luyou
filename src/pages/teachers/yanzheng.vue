...
<script>
  ...
  * */
  ...
  @Component({
    name: 'login',
    components: {
      SelBox,LoginBox,BtnBox,LoginTab,LoginThree
    }
  })
  export default class Login extends Vue {
    ...
    get checkBtnText(){
      let checkBtnState=this.checkBtnState;
      if(checkBtnState=='gray'||checkBtnState=='submit'){//初始化不可点击状态
        return '获取验证码';
    ...
    @Emit checkBtnCLick(){
      ...
      //获取验证码接口
      getMobileCpaCode({"mobile":this.phoneNumber,"areaCode":this.selVal.val}).then(res => {
        if(res.code=='0'){
