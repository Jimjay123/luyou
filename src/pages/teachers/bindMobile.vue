...
import { mobCapCheCaptcha, getMobileCpaCode } from "../../service/mainApi";
import { captchabindMobile } from "../../service/groupApi";
...
export default class BindMobile extends Vue {
  ...
  @Emit signClick() {
    ...
    captchabindMobile({
      ...
      if (res.code == "0") {
        //登录成功 跳转页面
        // this.mobileFun("bind", phoneNumber);
        let identifiedRoleType = res.identifiedRoleType;
        let token = res.token;
        if (identifiedType == 1 || identifiedType == 0) {
          sessionStorage.setItem("assent_token", token);
          this.$router.push({
            path: "/Uncertified"
          });
        } else if (identifiedType == 2) {
          sessionStorage.setItem("assent_token", token);
          this.$router.push({
            path: "/StudentMain"
          });
