...
<template>
  <div class="teaPreview">
    <!-- 老师端头部 start -->
    <TopHeader></TopHeader>
    <!-- 老师端头部 end -->

    <!-- 头部layer start -->
    <div class="topHeaderLayer"></div>
    <!-- 头部layer end -->
            ...
            <div v-if="item.showDialog===1">
              ...
              >
                <div class="bigBox">
                  ...
                  <div class="bottom">
                    <a class="goStudy" href="javascript:;" @click.stop="goPreview(item,index)">预览</a>
                  </div>
                </div>
...
import TopHeader from "../Header/headers";
import CallBackBtn from "../../pages/creator/sandTablePreview/callBackBtn";
import TitlePlugin from "../../pages/creator/sandTablePreview/titlePlugin";
import CourseInfo from "../../pages/creator/sandTablePreview/courseInfo";

import { stChannelData } from "../../service/studentApi";
...
export default class TeaPreview extends Vue {
  @TeachersStore.State("previewList") previewList; //老师端沙盘课程预览
  @TeachersStore.Action("previewListHandle") previewListHandle; //新曾或编辑 课程计划 相关参数  action修改方法
  //data
  get channelName() {
  ...
  created() {
    let channelId = this.$route.query.courseChannelId;
    stChannelData({ channelId: channelId })
      .then(res => {
        console.log(res);
        if (res.code === 0) {
          let previewList = cloneObj(this.previewList);
          previewList = res.data.stChannelData;
          this.previewListHandle({
            previewList: previewList
          });
          console.log(this.previewList);
