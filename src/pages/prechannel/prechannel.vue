      ...
      <el-main clearfix>
        <div class="Analysis">
          ...
          <div class="fx">
            ...
            <el-row>
              <el-button @click="Thirdparty(wrz,channelInfo,bundleList)">去预览</el-button>
            </el-row>
          </div>
        </div>
        <!-- 内容简介 -->
          ...
          <div class="ow" v-for="(item,index) in bundleList" :key="item.id">
            <div class="one">
              ...
              <div class="yll" v-for="items in item.courseList" :key="items.id">
                ...
                <el-row>
                  <el-button @click="Thirdparty1(wrz,items)">预览</el-button>
                </el-row>
              </div>
            </div>
          </div>
...
export default {
    ...
    courseChannelDetail({ courseChannelId: courseChannelId })
      .then(res => {
        if (res.code == "0") {
          ...
          // this.list = res.data.coursePartner;
        } else {
          console.log(res.msg);
        }
      })
  ...
  methods: {
    ...
    // 预览
    Thirdparty(wrz, data, bundleList) {
      ...
      if (availPreview == false && wrz == 1) {
        ...
        if (data.type === 12) {
          ...
          let courseId = bundleList[0].courseList[0].courseId;
          if (num === -1) {
            //tpe
            window.open(
              window.location.href.split("#")[0] +
