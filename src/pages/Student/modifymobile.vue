...
<template>
  <TeaDiaBox :show="show" :callback="callback">
    <img src="../../assets/img/close.svg" alt @click="closeBtn(false)" class="closeBtn" />
    <h3>修改手机号</h3>
    <div :class="inpBoxClass">
      ...
      <input
        ...
        disabled="true"
      />
      <i @click="delFun('originMobile')" v-show="getFocus=='error'" class="el-icon-error"></i>
    </div>
...
export default class BindMobile extends Vue {
  ...
  @Emit phoneChaInit() {
    let phoneNumber = this.originMobile;
    if (this.getFocus != "focus") {
      this.getFocus = false;
    }
  ...
  @Emit checkBtnCLick() {
    ...
    //获取验证码接口
    getMobileCpaCode({ mobile: this.originMobile, areaCode: this.selVal.val })
      .then(res => {
        if (res.code == "0") {
