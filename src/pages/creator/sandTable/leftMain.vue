<template>
  <div class="leftMain">
    <iframe
      ...
      frameborder="0"
      v-if="!sandTable.sceneServerUrl"
    ></iframe>
    <div class="dragIframe" @mousedown.prevent="moveIframe($event)" v-else>
          ...
          <div v-if="item.showDialog===1">
            ...
            >
              <div class="bigBox">
                ...
                <div class="bottom">
                  <div class="preview" @click.stop="goPreview(item,index,item.courseServerUrl)">
                    <img
                      class="jumpPre"
                      src="../../../assets/creator/sandTable/icon-preview.svg"
                      alt
                    />
                    <span class="jumpPre">预览</span>
                  </div>
                  <a
                    class="change"
                    href="javascript:;"
    ...
    <!-- 未上传图片 start -->
    <div class="btn" v-if="!sandTable.sceneServerUrl">
    ...
    <!-- change start -->
    <div class="changeBtn" v-if="sandTable.sceneServerUrl" @click.stop="clickFun">替换场景</div>
    <!-- change end -->
...
export default class LeftMain extends Vue {
  ...
  @SandTableMod.State("oldSandTable") oldSandTable; //沙盘课程频道-添加课程--对比课程
  @SandTableMod.Action("oldSandTableHandle") oldSandTableHandle; //沙盘课程频道-添加课程-对比课程
  ...
  stChaNewOpe(data) {
    stChannelNewOperate(data)
      .then(res => {
        // console.log(res);
        if (res.code == 0) {
          let oldSandTable = cloneObj(this.sandTable);
          let sandTable = res.data.currentStChannelData;
          sandTable.previousOperate = res.data.previousOperate;
          this.sandTableHandle({
            sandTable: sandTable
          });
          sandTable.stCourseList.map(item => {
            let obj = oldSandTable.stCourseList.find(
              d => d.courseId === item.courseId
            );
            if (obj) {
              item.showDialog = obj.showDialog || 0;
              item.showBg = obj.showBg || 0;
              item.ceilOrFloor = obj.ceilOrFloor || 0;
            }
          });
          this.oldSandTableHandle({
            oldSandTable: oldSandTable
          });
  ...
  // 预览课程
  @Emit goPreview(item, index, data) {
    // 沙盘npe||沙盘tpe
  ...
  created() {
    ...
    if (channelId == undefined) {
        ...
        .then(res => {
          ...
          if (res.code == 0) {
            ...
            this.$router.replace({
              ...
              query: {
                channelId: res.stChannelData.channelId
              }
            });
          } else {
            tipsFail(res.msg);
      ...
      getStChannelData({ channelId: channelId })
        .then(res => {
          // console.log(res);
          if (res.code == 0) {
            let sandTable = cloneObj(this.sandTable);
