...
<template>
  <TeaDiaBox :show="show" :callback="callback">
    ...
    <div :class="inpBoxClass">
      ...
      <div class="select">手机号：</div>
      <input
        v-bind:value="phoneNumber"
        v-text="ruleForm.mobile"
        class="noBgc"
        :placeholder="ruleForm.mobile"
      />
    ...
    <p>注：由于手机号即为平台账号，故修改手机号后，登录账号将一同修改</p>
    <!-- 提交按钮 start -->
    <modifyBox
      style="margin-top:13px;margin-bottom:28px;"
      :signClick="signClick"
      :signBtn="signBtn"
    ></modifyBox>
    <!-- 提交按钮 end -->
...
import { getMobileCpaCode, studentchangeMobile } from "../../service/mainApi";
...
import { membermemberTeacher } from "../../service/groupApi";
@Component({
  name: "BindMobile",
  components: {
    SelBox,
...
export default class BindMobile extends Vue {
    ...
    membermemberTeacher({})
      .then(res => {
        ...
        if (res.code == "0") {
          ...
          this.phoneNumber = this.ruleForm.mobile;
        } else {
          tipsFail(res.msg);
        }
      })
  ...
  @Emit checkBtnCLick() {
    ...
    let phoneNumber = this.ruleForm.mobile;
  ...
  @Emit signClick() {
    let phoneNumber = this.phoneNumber;
    let capNum = this.capNum;
    let vals = this.selVal.val;
    let serverCode = this.serverCode;
    this.$emit("showCityName", phoneNumber);
    ...
    studentchangeMobile({
      ...
      if (res.code == "0") {
        ...
        if (res.code === 603) {
          ...
          this.originMobile = this.newMobile;
          this.show = false;
        }
      } else {
