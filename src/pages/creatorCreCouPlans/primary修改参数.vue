      ...
      <div class="content-cli">
        <div class="top clearfix">
          <div class="left">
            ...
            >{{item.name}}</span>
            <!-- <span :class="{'item':true,'active':subjectId=='3'}" @click="tabClick('3')">心理学</span> -->
  ...
  @Emit selectFun(type, i, id) {
    ...
    if (type == 1) {
      ...
      if (this.temIndex !== null) {
        ...
        creatorClassList[this.temIndex].chapter.push(data);
      } else {
    ...
    courseListForChannel({
      ...
      if (res.code == 0) {
        ...
        if (this.temIndex === 0 || this.temIndex) {
          chapter = this.creatorClassList[this.temIndex].chapter;
          console.log(chapter);
        }
        // console.log(pageList);
        pageList.map(item => {
          let obj = chapter.find(ite => ite.courseId === item.courseId) || "";
          // console.log(obj);
          let type = false;
          if (obj) {
            type = obj.type;
        ...
        this.creList = cloneObj(creList);
      } else {
        tipsFail(res.msg);
      }
    });
    //获取专业列表
    creatorMajorList(data).then(res => {
      // console.log(res);
      if (res.code === 0) {
