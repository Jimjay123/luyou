  ...
  <div class="creatorCreCouPlans">
    <div class="header">
      <div class="left clearfix">
        ...
        <div class="saved">
          <img src="../../assets/creator/class-save.svg" alt />
          <span>今天 19 : 55 自动保存</span>
        </div>
      </div>
    ...
    <!-- 退出 -->
    <button class="exitBtn" @click="exit('pre')">退出</button>
    <!-- 退出结束 -->
...
import { saveCourseChannel } from "../../service/teachersApi";
...
export default class CreCouPlans extends Vue {
  ...
  // @Emit watchList() {
  // 监听creatorClassList
  // @Watch("creatorClassList", { immediate: true, deep: true }) testFun() {
  //   console.log("creatorClassList");
  //   clearInterval(this.timer);
  //   console.log(this.creatorClassList);
  //   // if (this.creatorClassList.length === 1) {
  //   //   console.log("添加第一个章节啦");
  //   //   if (this.creatorClassList.chapter.length === 1) {
  //   //     console.log("添加第一个section啦");
  //   //   } else {
  //   //     console.log("添加新section啦");
  //   //   }
  //   // } else if (this.creatorClassList.length >= 2) {
  //   //   console.log("又添加新章节啦");
  //   // } else {
  //   //   console.log("null");
  //   // }
    ...
    saveCourseChannel(data).then(res => {
      ...
      if (res.code === 0) {
        if (res.data == null) {
          ...
          //   list: res.data.bundleList
          // };
          // // creatorClassList = cloneObj(res.data.bundleList);
          // // creatorClassList[channelId] = res.data.channelId;
          // // creatorClassList.createDatetime = res.data.createDatetime;

          // // let creatorClassList = res.data;
          // // let createDatetime = creatorClassList.createDatetime;
          // // createDatetime = createDatetime.slice(11, 16);
          // //
          // // let creatorClassList = {};
          ...
          // //   createDatetime
          // // });
          // this.creatorClassListHandle({ creatorClassList: creatorClassList });
        }
      }
    });
  }
  ...
  @Emit goRelease() {
    ...
    saveCourseChannel(data).then(res => {
      console.log(res);
      if (res.code === 0) {
        this.courseChannelId = res.data.courseChannelId;
        this.$router.push({
  ...
  created() {
    ...
    // this.isHasList = this.creatorClassList.length === 0 ? false : true;
    this.isHasList = this.courseChannelId == "" ? false : true;
    console.log(this.creatorClassList);
    console.log(this.courseChannelId);
  ...
  updated() {
    console.log(this.creatorClassList);
    console.log(this.courseChannelId);
    if (!this.courseChannelId && this.creatorClassList.length != 0) {
      this.goSave();
      //
    } else {
      //
